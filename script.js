...
function loadApps() {
    apps = [
        ...
        },
        {
            id: 'calendar',
            title: 'Календарь',
            description: 'Планировщик и календарь',
        ...
        },
        {
            id: 'photos',
            title: 'Фотографии',
            description: 'Просмотр и редактирование фото',
            icon: 'fas fa-images',
            category: 'media',
            size: '12.4 MB',
            rating: 4.7,
        ...
        },
        {
            id: 'music',
            title: 'Музыка',
            description: 'Проигрыватель музыки',
            icon: 'fas fa-music',
            category: 'media',
            size: '8.7 MB',
            rating: 4.3,
...
function createLiveTiles() {
    ...
    const tiles = [
        { icon: 'fas fa-folder', title: 'Проводник', color: '#0078d7', wide: false },
        { icon: 'fas fa-globe', title: 'Браузер', color: '#107c10', wide: false },
        { icon: 'fas fa-shopping-bag', title: 'Магазин', color: '#e81123', wide: true },
        { icon: 'fas fa-cog', title: 'Настройки', color: '#744da9', wide: false },
        { icon: 'fas fa-music', title: 'Музыка', color: '#bf0077', wide: false },
        { icon: 'fas fa-images', title: 'Фотографии', color: '#008272', wide: false },
        { icon: 'fas fa-gamepad', title: 'Игры', color: '#f7630c', wide: true },
        { icon: 'fas fa-calendar-alt', title: 'Календарь', color: '#004b50', wide: false }
    ];
...
function initializeSystem() {
    ...
    if (savedAnimations) {
        ...
        document.getElementById('windowAnimations').value = savedAnimations;
    }
...
function openApp(appId) {
    ...
    if (window) {
        ...
        window.style.animation = animation;
        setTimeout(() => {
            window.style.animation = '';
        }, 400);
...
function centerWindow(window) {
    const rect = window.getBoundingClientRect();
    const isPositioned = window.style.left !== '' && window.style.top !== '';
    
    if (!isPositioned || (rect.left === 0 && rect.top === 0)) {
        window.style.left = (window.innerWidth / 2 - rect.width / 2) + 'px';
        window.style.top = (window.innerHeight / 2 - rect.height / 2) + 'px';
